...
| Evaluation | `evaluate_spatial.py` | renders in `outputs/rendered/` (generated by the GUI) | plots and CSV files in `outputs/evaluation/` |
...
It reads all `.wav` files from `outputs/rendered/` (the ones generated by the GUI) and produces:
- a **polar plot** for each file decoded to a loudspeaker layout (energy per loudspeaker),
  in `outputs/evaluation/polar/`;
- an **ITD/ILD plot** for each binaural file, plus an `itd_ild_summary.csv` summary,
  in `outputs/evaluation/itd_ild/`.

You can pass a different folder with `--rendered-dir`:
```
