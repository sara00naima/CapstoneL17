...
from spatial_pipeline.ambisonics.encoding.foa import (
    encode_mono_to_foa,
)
...
def test_foa_basic_matches_old_formula_front():
    ...
    got = encode_mono_to_foa(
        s,
        position,
        convention="basic",
    )

    expected = _expected_foa_old(
        s,
        position.azimuth,
        position.elevation,
        convention="basic",
...
def test_foa_rejects_unknown_convention():
    ...
    try:
        ...
        raise AssertionError(
            "Expected ValueError for invalid convention"
        )